...
export default async function DoctorCaseViewPage({ params }) {
  ...
  const isMedUrgency =
    caseRecord.urgency?.toLowerCase() === "medium" ||
    caseRecord.urgency?.toLowerCase() === "moderate";

  const badgeColor = isHighUrgency
    ? "from-red-600 to-red-900 border-red-500"
    : isMedUrgency
    ? "from-yellow-600 to-yellow-900 border-yellow-500"
    : "from-green-600 to-green-900 border-green-500";
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex justify-between items-center">
          <Link
            ...
            className="text-neon-cyan hover:text-white transition-colors text-sm font-medium flex items-center gap-2"
          >
            ← Back to Dashboard
          </Link>
        </div>
        ...
        <div className="bg-bg-secondary rounded-2xl shadow-xl overflow-hidden border border-border-subtle">
          ...
          {/* Header */}
          <div
            className={`p-8 text-white flex justify-between items-center bg-gradient-to-br ${badgeColor} relative overflow-hidden`}
          >
            ...
            <div className="relative z-10">
              ...
              {patientRecord ? (
                <p className="text-white/80 font-mono text-sm">
                  Patient: {patientRecord.name} | ID: {patientRecord.patientId} |
                  Age: {patientRecord.age || "N/A"}
                </p>
              ) : (
                <p className="text-white/80 font-mono text-sm text-red-200">
                  Patient Data Unavailable (ID: {caseRecord.patientId})
                </p>
              )}
            </div>

            <div className="text-right flex flex-col items-end relative z-10">
              <div className="text-xs uppercase tracking-wider font-semibold text-white/70 mb-1">
                Triage Urgency
              </div>
          ...
          <div className="p-8 space-y-8">
            ...
            <section className="bg-bg-card p-6 rounded-xl border border-border-subtle">
              ...
              <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle font-mono text-sm whitespace-pre-wrap text-text-secondary leading-relaxed">
                {caseRecord.clinical_note ||
                  "No clinical note generated. System may still be processing."}
              </div>
            </section>

            {/* AI Diagnosis */}
            {caseRecord.rag_diagnosis &&
            Array.isArray(caseRecord.rag_diagnosis) &&
            caseRecord.rag_diagnosis.length > 0 ? (
              <section className="bg-neon-cyan/5 p-6 rounded-xl border border-neon-cyan/20">
                ...
                <div className="space-y-4">
                  {caseRecord.rag_diagnosis.map((diag, i) => (
                    <div
                      key={i}
                      className="bg-bg-secondary p-5 rounded-lg border border-border-subtle"
                    >
              ...
              <section className="bg-bg-card p-6 rounded-xl border border-border-subtle opacity-60">
                <p className="text-sm text-text-muted italic">
                  No specific RAG diagnostics generated for this case.
                </p>
              </section>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <section className="bg-bg-card p-6 rounded-xl border border-border-subtle">
                ...
                {caseRecord.extracted_entities &&
                typeof caseRecord.extracted_entities === "object" &&
                Object.keys(caseRecord.extracted_entities).length > 0 ? (
                  <ul className="space-y-3 text-sm">
                    {Object.entries(caseRecord.extracted_entities).map(
                      ([key, vals]) => (
                        <li
                          key={key}
                          className="bg-bg-primary p-3 rounded-lg border border-border-subtle flex flex-col gap-1"
                        >
                          <span className="font-semibold text-neon-cyan uppercase text-xs tracking-wider">
                            {key}
                          </span>
            ...
            <section className="bg-bg-card p-6 rounded-xl border border-border-subtle">
              ...
              <div className="bg-bg-primary p-4 rounded-lg border border-border-subtle text-sm text-text-secondary">
                Patient specified Duration:
                <span className="font-semibold text-white ml-2">
                  {caseRecord.duration || "N/A"}
                </span>

                <br />

                Algorithm Temporal Info:
                <span className="font-mono text-xs ml-2">
                  {JSON.stringify(caseRecord.temporal_info || {})}
                </span>
              </div>
            </section>
