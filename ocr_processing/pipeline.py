...
from ocr_processing.input_handler import handle_input
...
def run_pipeline(source: str) -> dict:
    ...
    # Step 1: Input handling — detect type
    ...
    # Step 2: Convert to text based on input type
    if input_type == "audio":
        print(f"[Pipeline] Step 2: Audio-to-Text (ASR) — {content}")
        asr_result = transcribe_audio(content)
        transcript = asr_result["transcript"]
        print(f"[Pipeline] Transcript: {transcript[:100]}...")

    elif input_type == "image":
        print(f"[Pipeline] Step 2: Image-to-Text (OCR) — {content}")
        transcript = ocr_from_image(content)
        print(f"[Pipeline] OCR output: {transcript[:100]}...")

    elif input_type == "pdf":
        print(f"[Pipeline] Step 2: PDF-to-Text (OCR) — {content}")
